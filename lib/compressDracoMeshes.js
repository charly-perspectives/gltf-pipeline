 ...
 * @param {Number} [options.dracoOptions.compressionLevel=6] A value between 0 and 10 specifying the quality of the Draco compression. Higher values produce better quality compression but may take longer to decompress. A value of 0 will apply sequential encoding and preserve face order.
 ...
 * @param {Number} [options.dracoOptions.quantizeColorBits=10] A value between 0 and 30 specifying the number of bits used for color attributes. Lower values produce better compression, but will lose precision. A value of 0 does not set quantization.
...
function compress(gltf, options, encoderModule, decoderModule) {
    ...
    if (defined(quantizationVolume)) {
        ...
        const min = array1.Fill( Number.POSITIVE_INFINITY);
        const max = array2.Fill( Number.NEGATIVE_INFINITY);
    ...
    ForEach.mesh(gltf, function(mesh) {
        ForEach.meshPrimitive(mesh, function(primitive) {
            ...
            if (defined(primitive.targets) || primitive.mode === WebGLConstants.TRIANGLE_STRIP) {
